...
export class PhysiologyStore {
  ...
  status(scope:SceneScope,options:{readerId?:string;nowMs?:number}={}){
    const state=this.dependencies.state(scope),stored=this.configuration(scope),readerId=options.readerId??'player';
    const clock=this.clock(scope,options.nowMs??Date.now());
    ...
    const corrections=this.corrections(scope);
    const before=corrections.filter(item=>item.afterSourceId===null);
    for(const correction of before)this.applyCorrection(projected,correction,readerId);
    for(const source of state.sources){
      if(source.status==='accepted'&&source.processing==='ready')for(const operation of source.analysis?.physiologyOperations??[])
        this.applyOperation(projected,operation,readerId);
      for(const correction of corrections.filter(item=>item.afterSourceId===source.id))this.applyCorrection(projected,correction,readerId);
    }
    const knownSourceIds=new Set(state.sources.map(source=>source.id));
    for(const correction of corrections.filter(item=>item.afterSourceId!==null&&!knownSourceIds.has(item.afterSourceId)))this.applyCorrection(projected,correction,readerId);
    const characters=[...projected.values()].map(character=>projectCharacter(character,config,clock.timeMs));
  ...
  context(scope:SceneScope,characterId:string,nowMs=Date.now()):string{
    const status=this.status(scope,{readerId:characterId,nowMs});
    ...
    return `\n[XLDB 虚拟角色生理状态] 仅用于扮演角色 ${character.name}，不是现实用户身体资料。以下是已接受来源或用户纠正后的可知状态；unknown 不得补写。${JSON.stringify(promptState)}`;
  }
  ...
  private applyOperation(characters:Map<string,MutableCharacter>,operation:PhysiologyOperation,readerId:string){
    const character=characters.get(operation.characterId);if(!character||!operation.readers.includes(readerId))return;
  ...
  private applyCorrection(characters:Map<string,MutableCharacter>,correction:StoredCorrection,readerId:string){
    const character=characters.get(correction.characterId);if(!character||!correction.readers.includes(readerId))return;
...
interface MutableCharacter {
  characterId:string;name:string;needs:Map<PhysiologyNeed,{stage:typeof needStages[number];atMs:number|null;sleeping:boolean;corrected:boolean}>;
  effects:Map<EffectKind,{kind:EffectKind;severity:EffectSeverity;detail:string;atMs:number|null;corrected:boolean}>;
...
function projectCharacter(character:MutableCharacter,config:PhysiologyConfiguration,timeMs:number|null){
  const needs=Object.fromEntries([...character.needs].filter(([need])=>config.dailyNeeds.includes(need)).map(([need,value])=>{
    ...
    return [need,{stage,sleeping:value.sleeping,anchorTimeMs:value.atMs,corrected:value.corrected}];
  }));
