...
import { configsOf, integer, messageOf, object, scopeOf, text } from './core/types.ts';
...
export function loadConfig(filename: string): Configurations {
  return fs.existsSync(filename) ? configsOf(JSON.parse(fs.readFileSync(filename, 'utf8'))) : configsOf({});
}
export function saveConfig(filename: string, config: Configurations) {
    ...
    try {
      ...
      switch (request.url) {
        ...
        case '/v1/scene/profile-controls': {
          const subject=requireSubject(),controls=core.authority.scene.userModel.setControls(subject,object(input.patch),integer(input.expectedRevision));
        ...
        case '/v1/scene/profile-correct': {
          ...
          const result=core.authority.scene.userModel.correctEntry(subject,text(input.id,200),{...correction,claim:text(correction.claim,1000)});
        ...
        case '/v1/scene/profile-delete': {
          const subject=requireSubject();core.authority.scene.userModel.deleteEntry(subject,text(input.id,200));
        ...
        case '/v1/scene/dashboard': return send(200,sceneDashboard(core.authority.scene,scope));
